...
import com.example.carwashee.model.ProdutoRemocao;
...
public class RemoverProdutoServlet extends HttpServlet {
    ...
    protected void doDelete(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            ProdutoRemocao produtoRemocao = gson.fromJson(json, ProdutoRemocao.class);

            if (produtoRemocao.getTipoProduto() == null || produtoRemocao.getTipoProduto().isEmpty()) {
                response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
                response.getWriter().write(gson.toJson("Dados de produto inválidos."));
                return;
            }

            boolean sucesso = produtoDAO.removerProduto(produtoRemocao.getTipoProduto());

            if (sucesso) {
                response.setStatus(HttpServletResponse.SC_OK);
                response.getWriter().write(gson.toJson("Produto removido com sucesso."));
            } else {
            ...
            response.getWriter().write(gson.toJson("Erro ao remover produto: " + e.getMessage()));
        }
    }
}
